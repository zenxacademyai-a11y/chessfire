      ...
      <div className="glass-panel rounded-xl w-48 md:w-56 overflow-hidden"
        ...
        <div className="px-3 py-2 border-b border-border/50 flex items-center gap-2">
          ...
          {canUndo && (
            <button
              onClick={onUndo}
              className="ml-1 p-1 rounded-md text-muted-foreground hover:text-foreground hover:bg-muted/60 transition-all"
        ...
        {isReplaying && (
          <div className="px-3 py-1.5 bg-primary/10 border-b border-primary/20 flex items-center justify-between">
            <span className="text-[10px] font-bold text-primary tracking-wide">VIEWING MOVE {viewingMoveIndex! + 1}</span>
            <button
              ...
              {movePairs.map((pair, i) => (
                <div key={i} className="flex items-stretch text-xs">
                  ...
                  <MoveCell
                    entry={pair.fire}
                    color="fire"
                    isActive={pair.fire ? pair.fire.index === viewingMoveIndex : false}
                    isLatest={pair.fire ? pair.fire.index === moves.length - 1 && viewingMoveIndex === null : false}
                    onClick={onMoveClick}
                  />
                  ...
                  <MoveCell
                    entry={pair.ice}
                    color="ice"
                    isActive={pair.ice ? pair.ice.index === viewingMoveIndex : false}
                    isLatest={pair.ice ? pair.ice.index === moves.length - 1 && viewingMoveIndex === null : false}
                    onClick={onMoveClick}
                  />
                </div>
              ))}
...
function MoveCell({ entry, color, isActive, isLatest, onClick }: {
  entry?: { record: MoveRecord; index: number };
  color: PieceColor;
  isActive: boolean;
  isLatest: boolean;
  ...
  return (
    <div
      className={`flex-1 px-2 py-1.5 flex items-center gap-1 min-h-[28px] cursor-pointer transition-all ${
        isActive
      ...
      title={`View board after this move`}
    >
      <span className={`${colorClass} text-sm leading-none`}>
        {pieceUnicode(record.pieceType, color)}
      </span>
